...
void task1Entry(void * param) 
{
	...
	//7.1 测试:
	tMboxInit(&mbox1,  (void*)mbox1MsgBuffer, 20); //容量是20个 //我替换了老师的:tMboxInit(&mbox1, (void*)mbox1MsgBuffer, 20); //我测试了, 两者都可以
...
void task2Entry(void * param) 
{
	for(;;){
		...
		{
			//消息的转换
			uint32_t value = (uint32_t)*(uint32_t*)msg2; //todo, 老师原有的: uint32_t value = *(uint32_t*)msg2; 我发现改了也没有问题
			task2Flag = value;
			tTaskDelay(1);
		}
