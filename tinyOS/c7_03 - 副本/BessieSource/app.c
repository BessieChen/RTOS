...
void task4Entry(void * param) 
{
	...
	for(;;){
		...
		uint32_t* msgTask4;
		tMboxWait(&mbox2, (void*)msgTask4, 0);//感觉老师写错了把? tMboxWait(&mbox2, (void*)&msgTask4, 0); //这里是从mbox中读取一条信息, 将信息的地址存到msgTask
		task4Flag = *msgTask4; //解析地址存放的值, 也就是我们插入的Msg[i] = i, 因为是第一个所以msg[0] = 7, 这个task4Flag是7
		
		tMboxFlush(&mbox2); //上一句就读取了一个msg, 可是这一句结束, 所有msg都会被删除
		/*
		task4Flag = 0;
		tTaskDelay(1);
		task4Flag = 1;
