...
//3.6 taskTable¾ÍÊÇtListÊı×éÁË, Ò²¾ÍÊÇÃ¿¸öÔªËØ¶¼ÊÇtList //3.4 taskTableÏÖÔÚÀïÃæÓĞTINYOS_PRIO_COUNT¸öÔªËØ: tTask* taskTable[TINYOS_PRIO_COUNT]; 
...
tTask tTask2;
tTask tTaskIdle; //2.4
tTask tTask3; //3.7 ÎªÁË²âÊÔ, ÎÒÃÇĞèÒª½«task2ºÍtask3µÄÓÅÏÈ¼¶ÉèÖÃÎªÍ¬Ò»ÓÅÏÈ¼¶
...
//3.6 ¶¨ÒåÑÓÊ±¶ÓÁĞ
tList tTaskDelayedList; //×¢Òâ, ÑÓÊ±½ÚµãdelayNodeÊÇÃ¿¸ötTaskÀïÃæÓĞµÄ³ÉÔ±, µ«ÊÇÑÓÊ±¶ÓÁĞÈ´ÊÇmain.cÀïÃæµÄÈ«¾Ö±äÁ¿. //×¢Òâ, Õâ¸öÊÇÒ»¸ötList, ¶ø²»ÊÇtList*, ÒòÎªÎÒÃÇÕâÀïĞèÒªµÄÊÇÒ»¸ö½á¹¹Ìå, ¶ø²»ÊÇÒ»¸öµØÖ·
...
//3.7 ³õÊ¼»¯tasktable[] 3.4 ËäÈ»ÊÇ³õÊ¼»¯µ÷¶ÈËø,µ«ÊÇÕâÀïÒ²°üÀ¨ÁËbitmapµÄ³õÊ¼»¯,ÒòÎªmain()µÚÒ»¸öµ÷ÓÃµÄ¾ÍÊÇÕâ¸öº¯Êı.  µ÷¶ÈËøÇø±ğÓÚÁÙ½çÇø:ÁÙ½çÇøÊÇÊ¹ÓÃ__get_primask()À´¹Ø±ÕÖĞ¶Ï,Ò²¾ÍÊÇÊ¹ÓÃÁËARMCM3.hÎÄ¼şµÄº¯Êı,µ«ÊÇµ÷¶ÈËøÊ¹ÎÒÃÇ×Ô¼º¶¨ÒåµÄ
...
//3.6 ÑÓÊ±¶ÓÁĞtTaskDelayedListµÄ³õÊ¼»¯, ×¢ÒâÕâÀïµÄ²ÎÊı²»ĞèÒªÊÇtList*, ÒòÎªtTaskDelayedList¾ÍÊÇÈ«¾Ö±äÁ¿
void tTaskDelayedInit(void)
{
	tListInit(&tTaskDelayedList);
}

//3.6 ½«ÈÎÎñ¼ÓÈëÑÓÊ±¶ÓÁĞ, ²¢ÉùÃ÷ÈÎÎñĞèÒªÑÓÊ±¶à¾Ã
void tTimeTaskWait(tTask* task, uint32_t ticks)
{
	...
	//task¼ÓÈëÑÓÊ±¶ÓÁĞ
	tListAddLast(&tTaskDelayedList, &(task->delayNode)); //×¢ÒâÊÇ¼ÓÈëtaskµÄdelayNodeµØÖ·,Ò²¾ÍÊÇtNodeÖ¸Õë
}

//3.6 µ±delayTicks == 0Ö®ºó, ½«ÈÎÎñ´ÓÑÓÊ±¶ÓÁĞÖĞÉ¾³ı
void tTimeTaskWakeUp(tTask* task)
{
	//taskµÄĞŞ¸Ä
	task->state &= ~(TINYOS_TASK_STATE_DELAYED); //×¢ÒâÕâÀïÖ»ÊÇ¸Ä±äµÚ¶şÎ», ½«µÚ¶şÎ»ÉèÖÃ³É0
	
	//task´ÓÑÓÊ±¶ÓÁĞÉ¾³ı
	tListRemove(&tTaskDelayedList, &(task->delayNode));
}

//3.7 ĞŞ¸Ä //3.6 ¼ÓÈë¾ÍĞ÷¶ÓÁĞ: µ±task´ÓÑÓÊ±¶ÓÁĞÉ¾³ı, ÎÒÃÇ¾Í°Ñtask¼ÓÈë¾ÍĞ÷¶ÓÁĞ: Ò²¾ÍÊÇ¼ÓÈëtaskTable, ²¢ÇÒÉèÖÃtBitmap
void tTaskSchedReady(tTask* task)
{
	//3.7
	tListAddFirst(&(taskTable[task->prio]), &(task->linkNode)); //µÚÒ»¸ö²ÎÊıÊÇtList*, µÚ¶ş¸öÊÇtNode*
	//3.7 Èç¹û·¢ÏÖÕâ¸ötaskÊÇtasktable[]ÖĞµÄµÚÒ»¸ötask, ¾ÍÒª½«bitmapµÄ¶ÔÓ¦Î»ÖÃÉèÖÃ³É1
	if(tListCount(&taskTable[task->prio]) == 1) //ÎÒµÄbug, ÊÇ==1, ËµÃ÷¸Õ¼Ó½øÈ¥µÄÊÇµÚÒ»¸ö!
	{
		tBitmapSet(&taskPrioBitmap, task->prio);
	}
}

//3.7 ĞŞ¸Ä //3.6 µ±taskĞèÒªÑÓÊ±, »òÕßÆäËûÇé¿öÒª½«task´Ó¾ÍĞ÷¶ÓÁĞÖĞÉ¾³ı
void tTaskSchedUnReady(tTask* task)
{
	//3.7
	tListRemove(&(taskTable[task->prio]), &(task->linkNode)); //µÚÒ»¸ö²ÎÊıÊÇtList*, µÚ¶ş¸öÊÇtNode*
	//×¢Òâ, ÕâÀï²»Òª½«linkNode¼ÓÈëµ½taskTable[prio]µÄÄ©Î², ÒòÎªÕâ¸öº¯ÊıÊÇ°Ñtask´Ó¾ÍĞ÷¶ÓÁĞÉ¾³ı, ¶ø²»ÊÇÊ²Ã´Ê±¼äÆ¬µÄÂÖ×ª
	//3.7 Èç¹û·¢ÏÖtasktable¿ÕÁË, ¾ÍÒª½«bitmapµÄ¶ÔÓ¦Î»ÖÃÉèÖÃ³É0
	if(tListCount(&taskTable[task->prio]) == 0)
	{
		tBitmapClear(&taskPrioBitmap, task->prio);
	}
}
//3.7 ¼ÓÈëÊ±¼äÆ¬ÂÖ×ªµÄ´¦Àí, ÒòÎªÃ¿¸ösystick, ËùÓĞµÄÍ¬Ò»¸öÓÅÏÈ¼¶µÄËùÓĞÈÎÎñµÄÊ£ÓàÊ±¼äÆ¬¶¼Òª¼õÒ». 3.6 ĞèÒªĞŞ¸Ä, ÒòÎªÎÒÃÇĞèÒªÉ¨ÃèµÄÊÇÑÓÊ±¶ÓÁĞÁË, Ò²¾ÍÊÇtList tTaskDelayedList. 3.4 ĞèÒªĞŞ¸Ä,Ã¿´Îsystick¶¼ĞèÒª°ÑtaskTableÖĞµÄÃ¿Ò»¸öÈÎÎñµÄdelayTicks¼õÒ» // 2.4 ÒòÎªÏÖÔÚÃ¿¸öÈÎÎñ¶¼ÓĞÒ»¸ö¼ÆÊıÆ÷delayTicks, ÏÖÔÚÉè¼ÆÒ»¸öº¯Êı,ÄÜ¹»µİ¼õdelayTicks(Ïàµ±ÓÚÈíÑÓ³Ù)
//ËùÒÔsystick·¢Éú,»áÓĞÁ½¼şÊÂÇé: 1. ÑÓÊ±¶ÓÁĞµÄtaskµÄÑÓÊ±Ê±¼ä¶¼¼õÒ», 2. ¾ÍĞ÷¶ÓÁĞµÄtaskµÄÊ±¼äÆ¬¶¼¼õÒ»
void tTaskSysTickHandler()
{
	//3.6 ÕâÊÇÖ®ºófor loopĞèÒªÓÃµ½µÄ:
	tNode* node;
	uint32_t status = tTaskEnterCritical();
	//3.6 É¾³ı3.4²¿·Ö:
	/*
	...
	*/
	...
	//3.6 ÒòÎªtTaskDelayListÀïÃæ´æµÄ¶¼ÊÇÕıÔÚÑÓÊ±µÄtaskµÄ³ÉÔ±tNode delayNode, ËùÒÔÎÒÃÇ¾Í¿ÉÒÔºÜÈİÒ×ÕÒµ½ÕıÔÚÑÓÊ±µÄtask, ¾Í²»ÓÃÏñ3.4ÀïÃæµÄ, È«²¿task¶¼±éÀúÒ»±é
	// ÕÒµ½ÕıÔÚÑÓÊ±µÄtaskÖ®ºó,¾Í¿ÉÒÔ¶ÔËûÃÇµÄdelayTicks - 1, Èç¹û·¢ÏÖtick == 0, ¾Í¿ÉÒÔ°ÑÄÇ¸ötask ´ÓÑÓÊ±¶ÓÁĞÉ¾³ı, ²¢ÇÒ¼ÓÈë¾ÍĞ÷¶ÔÁĞ
	// ÕâÀïĞèÒª×¢ÒâµÄ, µÄÈ·ÈİÒ×³ö´í: ÄÇ¾ÍÊÇtTaskDelayedListÊÇÒ»¸ötList, ¶ø²»ÊÇtList*, ËùÒÔÎÒÃÇĞèÒªÓÃ. : tTaskDelayedList.headNode, ¶ø²»ÊÇÓÃ-> : ËùÒÔtTaskDelayedList->headNodeÊÇ´íµÄ
	for(node = tTaskDelayedList.headNode.nextNode; node != &(tTaskDelayedList.headNode); node = node->nextNode)
	{
		//Çó³öÄÇ¸ötaskµÄµØÖ·
		tTask* task = (tTask*)tNodeParentAddr(tTask, delayNode, node); //µÚÒ»¸ö²ÎÊı: ½á¹¹Ìå, µÚ¶ş¸ö: ½á¹¹ÌåµÄ³ÉÔ±, µÚÈı¸ö: ¸Ã³ÉÔ±µÄÊµ¼ÊµØÖ·. Çó: ½á¹¹ÌåµÄÊµ¼ÊÊ×µØÖ·

		//½«taskµÄdelayTIckµİ¼õ
		//Èç¹ûtick == 0, ´ÓÑÓ³Ù¶ÓÁĞÒÆ³ı(tTaskDelayedList, task->state), ¼ÓÈë¾ÍĞ÷¶ÓÁĞ(bitmap, taskTable, task->state)
		if(--task->delayTicks == 0)
		{
			tTimeTaskWakeUp(task); //ÔÚÕâÀï, nextTask»áµÈÓÚÕâ¸ötask
			tTaskSchedReady(task);
		}
	}
	
	//3.7 ¾ÍĞ÷¶ÓÁĞµÄtaskµÄÊ±¼äÆ¬¶¼¼õÒ»
	if(--currentTask->slice == 0)
	{
		//½«linkNode´ÓtListµÄµÚÒ»Î»É¾³ı
		tListRemoveFirst(&(taskTable[currentTask->prio]));
		//½«linkNode·Åµ½Ä©Î²
		tListAddLast(&(taskTable[currentTask->prio]), &(currentTask->linkNode));
		//ÖØÖÃcurrentTaskµÄÊ±¼äÆ¬
...
//3.6 ĞèÒªĞŞ¸Ä //3.4 ÕâÀïĞèÒª½«ĞŞ¸Ä //2.4 ÊµÏÖÑÓÊ±º¯Êı, ÒòÎªÕâ¸öº¯Êı¿Ï¶¨ÊÇcurrentTaskµ÷ÓÃµÄ,ËùÒÔÀïÃæÖ±½ÓĞ´currentTask->xxx
void tTaskDelay(uint32_t delay)
{
	...
	*/
	...
	//3.6 ½«task·ÅÈëÑÓÊ±¶ÓÁĞ, ½«task´Ó¾ÍĞ÷¶ÓÁĞÖĞÉ¾³ı
	tTimeTaskWait(currentTask, delay); //×¢Òâ, ÒòÎªÎÒÃÇÕâÀïÃ»ÓĞtask´«Èë, ¶øÊÇ´«ÈëcurrentTask, ÒòÎªÕâ¸öº¯Êı¿Ï¶¨ÊÇcurrentTask´«ÈëµÄ
...
void task2Entry(void * param) 
{
	for(;;){
		task2Flag = 0;
		//3.7 É¾³ı tTaskDelay(1); ÒòÎªtTaskDelay()»áÖ÷¶¯ÈÃ³ö×ÊÔ´,µ«ÊÇÎÒÃÇÕâÀïĞèÒªtask2Ò»Ö±Õ¼ÓÃ×ÊÔ´,È»ºó¾ÍÀ´¼ì²âÎÒÃÇµÄÊ±¼äÆ¬ÊÇ·ñÓĞÓÃ,Èç¹ûÓĞÓÃ,task2¼´±ãÏëÒ»Ö±Õ¼ÓĞ×ÊÔ´Ò²²»×ö²»µ½
		//3.7 ÎªÁËÈÃtask2Ò»Ö±Õ¼ÓÃ×ÊÔ´,Ê¹ÓÃdelay()
		delay(0xFF);
		task2Flag = 1;
		//3.7 É¾³ı: tTaskDelay(1);
		//3.7 ÎªÁËÈÃtask2Ò»Ö±Õ¼ÓÃ×ÊÔ´,Ê¹ÓÃdelay()
		delay(0xFF);
...
void taskInit(tTask* task, void (*func)(void*), void* param, uint32_t prio, tTaskStack* stack )
{
	...
	*(--stack) = (unsigned long) 0x4;
	...
	//3.6 ÑÓÊ±½Úµã ºÍ stateµÄ³õÊ¼»¯
	task->state = TINYOS_TASK_STATE_READY; // ¸Õ¿ªÊ¼¾ÍÊÇ¾ÍĞ÷×´Ì¬, Ã»ÓĞÑÓÊ±
	tNodeInit(&(task->delayNode)); //×¢Òâ, delayNodeÊÇÒ»¸ötNode, µ«ÊÇtNodeInit(tNode*), ËùÒÔÎÒÃÇÒªÈ¡µØÖ·
	
	//3.7 ³õÊ¼»¯Ê±¼äÆ¬
	task->slice = TINYOS_SLICE_MAX; //Ò²¾ÍÊÇËµ, ÏÖÔÚÕâ¸ötask¿ÉÒÔÖ´ĞĞÕâÃ´³¤µÄÊ±¼äÁË
	
	//3.7 ³õÊ¼»¯linkNode
	tNodeInit(&(task->linkNode)); //ÒòÎªtaskÊÇtTask*, È»ºólinkNodeÊÇtNode, µ«ÊÇĞèÒªµÄ²ÎÊıÊÇtNode*, ËùÒÔ¼ÇµÃ¼ÓÉÏ&
	
	//3.7 ½«linkNode¼ÓÈëtaskTableµÄ¶ÔÓ¦ÔªËØÖĞ Ïàµ±ÓÚÖ®Ç°µÄ taskTable[prio] = task //3.4 ³õÊ¼»¯Ò»¸ötaskÖ®ºó, Òª°Ñtask·Åµ½taskTableµÄÏàÓ¦Î»ÖÃ
	//ÒÔÇ°µÄÉ¾³ı: taskTable[prio] = task; //×¢Òâ,ÕâÀïµÄtask±¾Éí¾ÍÊÇÒ»¸öµØÖ·,ËùÒÔ²»ÒªĞ´³É = &task;
	tListAddFirst(&(taskTable[task->prio]), &(task->linkNode)); //ÒòÎªµÚÒ»¸ö²ÎÊıÊÇtLink*, µÚ¶ş¸ö²ÎÊıÊÇtNode*, ËùÒÔ¶¼ÊÇ´«ÈëµØÖ·
	

}

//3.7 ĞèÒªĞŞ¸ÄÁË //3.4 ·µ»Øµ±Ç°×î¸ßÓÅÏÈ¼¶µÄÈÎÎñ
tTask* tTaskHighestTaskReady(void) //×¢Òâ,ÕâÀï²»ĞèÒª´«Èë&taskPrioBitmap×÷Îª²ÎÊı,ÒòÎªÄ¬ÈÏÊÇÈ«¾Ö±äÁ¿. µ±È»ÄãÏë¸Ä³ÉÒªÊäÈë²ÎÊıµÄÒ²¿ÉÒÔ
{
	//3.7 ÕâÒ»²½²»±ä, »¹ÊÇÍ¨¹ıbitmapÖªµÀ×î¸ßÓÅÏÈ¼¶ÊÇµÚ¼¸¸öÓÅÏÈ¼¶
	uint32_t highestPrio = tBitmapGetFirstSet(&taskPrioBitmap);
	
	//3.7 È»ºó´ÓtaskTableÖĞÕÒµ½¶ÔÓ¦µÄÓÅÏÈ¼¶, Ò²¾ÍÊÇÒ»¸ötList, ÎÒÃÇĞèÒª´ÓÕâ¸ötListÖĞÕÒ³öµÚÒ»¸ötNode,(×¢Òâ²»ÊÇtNode*) Õâ¸ötNodeËäÈ»´æ×ÅÁ½¸ötNode*, µ«ÊÇtNode±¾ÉíÒ²ÓĞ×Ô¼ºµÄµØÖ·(ÆäÊµtListµÄheadNode.nextNode´æµÄ¾ÍÊÇÕâ¸ötNode×Ô¼ºµÄµØÖ·), ÎÒÃÇÍ¨¹ıÕâ¸öµØÖ·, ÕÒµ½Õâ¸ötNodeËùÊôµÄtTask, ½«Õâ¸ötTaskµÄÊ×µØÖ··µ»Ø
	tNode* linkNodeAddr = tListFirst(&(taskTable[highestPrio])); //×¢Òâ·µ»ØµÄ¾ÍÊÇtNode*
	
	//3.7 ÎÒÃÇÖªµÀlinkNodeµÄÊµ¼ÊµØÖ·, ÏëÇó³ötTaskµÄÊµ¼ÊÊ×µØÖ·
...
int main()
{
	...
	taskInit(&tTask3, task3Entry, (void*)0x00000000, 1, &tTask3Env[1024]);  //3.7 ³õÊ¼»¯task3, ×¢ÒâÓÅÏÈ¼¶ºÍtask2Ò»Ñù
	...
	//3.7 É¾³ı taskTable[0] = &tTask1;
	//3.7 É¾³ı taskTable[1] = &tTask2;
