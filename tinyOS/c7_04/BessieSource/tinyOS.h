...
//5.2 错误码
typedef enum _tError //对应着uint32_t result // result: 最后这个参数是告诉task他们从eventECB中移除的原因是什么
{
	tErrorNoError = 0, //说明task从eventECB中移除, 是因为没有错误, 就是等到了资源
	tErrorTimeout, //说明task从eventECB中移除, 是因为延时时间到了, 超时了
	tErrorResourceUnavailable, //没有资源, task也不打算继续等
	tErrorDel, //说明task从eventECB中移除, 是因为被删除
	tErrorResourceFull, //7.2 邮箱已满
}tError;
