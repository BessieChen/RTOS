...
{
	//3.3 ÎÒÃÇÀ´¼ì²âÒ»ÏÂÎÒÃÇµÄbitmapµÄº¯ÊıÊÇ·ñÔËĞĞÕıÈ·: 
	//Ê×ÏÈ¶¨Òå:
	tBitmap bitmap;
	int i; //×¢Òâ, ÈİÒ×´íµÄµØ·½: ÎÒÃÇÒª½«iÉèÖÃ³Éint,¶ø²»ÊÇuint32_t(unsigned intµÄÒâË¼), ²»ÒªÒòÎªtBitmapPosCount()·µ»ØµÄÊÇuint32_t,¾Í½«iÉèÖÃ³Éuint32_t
					//uint32_t i, Èç¹ûÔÚµİÔöµÄÊ±ºò¿ÉÒÔ,µ«ÊÇÔÚµİ¼õµÄÊ±ºò, i--, ÓÀÔ¶¶¼ÊÇi > 0 , ¾ÍÊÇ»áÎŞÏŞÑ­»·!
	uint32_t flag; 
	...
	//3.3 ÎÒÃÇÀ´¼ì²âÒ»ÏÂÎÒÃÇµÄbitmapµÄº¯ÊıÊÇ·ñÔËĞĞÕıÈ·: 
	//3.3 Ê×ÏÈ½«bitmap³õÊ¼»¯
	tBitmapInit(&bitmap); //×¢Òâ,ÒòÎªĞèÒªbitmapÖ¸Õë,ËùÓĞÕâÀïÖ±½Ó´«ÈëÁËbitmapµÄµØÖ·
	//3.3 ´Ó×î¸ßbit¿ªÊ¼ÉèÖÃ³É1, Èç¹û±¨´í, »á×ßµ½ifError()
	for(i = tBitmapPosCount() - 1; i >= 0; i --) //×¢Òâ: Ê×ÏÈÉèÖÃµÄÊÇµÚ31Î» //×¢Òâ!ÊÇtBitmapPosCOunt(), ÊÇº¯Êı°¡!! ²»ÊÇtBitmapPosCOunt°¡!!
	{
		tBitmapSet(&bitmap, i);
	...
	//3.3 ´Ó×îµÍbit¿ªÊ¼ÉèÖÃ³É0, Èç¹û±¨´í, »á×ßµ½ifError()
	for(i = 0; i < tBitmapPosCount(); i++)
	{
		tBitmapClear(&bitmap, i);
		flag = tBitmapGetFirstSet(&bitmap);
		if(flag != (i + 1) ) //×¢Òâ, ÕâÀïÊÇi+1, ÒòÎªÊÇÉ¾³ıÁËµÚiÎ»µÄÔªËØ,ËùÒÔflagÊÇi+1. Èç¹ûÉ¾³ıµÄÊÇµÚ31Î»ÔªËØ, ±ä³ÉbitmapÈ«0, ÄÇÃ´¸ÕºÃÒ²ÊÇ·µ»Ø32 == 31+1
		{
			ifError();
		}
	}
