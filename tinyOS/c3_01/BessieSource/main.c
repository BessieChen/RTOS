...
//3.1 Ìí¼ÓÈÎÎñºÍÖĞ¶Ï¶¼ÄÜ·ÃÎÊµÄÈ«¾Ö±äÁ¿
int tickCounter;
...
//3.1 ¼ÇµÃÒª¶ÔÕâ¸öº¯Êı½øĞĞĞŞ¸Ä, ¾ÍÊÇÕû¸öº¯Êı¶¼ÒªÓÃÁÙ½çÇø±£»¤
//ÒòÎªÕâ¸ötTaskSched()º¯Êı¿ÉÒÔ±»ÈÎÎñµ÷ÓÃ,Ò²¿ÉÒÔ±»ÖĞ¶Ïµ÷ÓÃ. ÎÒÃÇÒª±£»¤µÄÊÇÈ«¾Ö±äÁ¿: taskTable »¹ÓĞnextTaskµÈ
void tTaskSched()
{
	//2.4ÒÔÏÂµÄ¶¼ÒªÉ¾µô
	/*
	...
	*/
	...
	//3.1 Ê×ÏÈÊÇ¼ÓÉÏÁÙ½çÇø, ¹Ø±ÕÖĞ¶Ï
	uint32_t status = tTaskEnterCritical();
	//Ö®ºó¼ÇµÃÔÚÃ¿Ò»¸ö¿ÉÄÜµÄ³ö¿Ú, ÀıÈçreturnÖ®Ç°, ÍË³öÁÙ½çÇø
	...
	//3.1 ¿ÉÄÜµÄ³ö¿Ú
	tTaskExitCritical(status);
	//Ë¼¿¼: ÎªÊ²Ã´ÊÇ¼ÓÔØtTaskSwitch(): tTaskSwitchĞèÒªpendSVÒì³£, ¹Ø±ÕÖĞ¶ÏµÄÈ·²»ÄÜÖ´ĞĞpendSV,µ«ÊÇµ½ÕâÒ»¾ä¿ªÆôÖĞ¶ÏµÄÊ±ºò,¾Í»áÖ´ĞĞpendSVÁË,Èç¹ûÓĞµÄ»°
	//¾ßÌåÄÚÈİ²Î¼û:
...
//3.1 ¼ÇµÃÒª¶ÔÕâ¸öº¯Êı½øĞĞĞŞ¸Ä, ¾ÍÊÇÕû¸öº¯Êı¶¼ÒªÓÃÁÙ½çÇø±£»¤, ÒòÎªtaskTableĞèÒª±»±£»¤
void tTaskSysTickHandler()
{
	//3.1 ÒòÎªtaskTableĞèÒª±»±£»¤, ÉèÖÃÁÙ½çÇø, ¹Ø±ÕÖĞ¶Ï
	uint32_t status = tTaskEnterCritical();
	int i;
	for(i = 0; i < 2; i++)
	{
	...
	//3.1 ×¢Òâ,ÕâÀï²»ĞèÒª½øÈëÁÙ½çÇø,ÒòÎªÕâÀï¼ÙÉè:Ã»ÓĞ¸ü¸ßÓÅÏÈ¼¶µÄÖĞ¶ÏÈ¥·ÃÎÊÕâ¸ötickCount;
	//3.1 ÔÚÕâÀï,ÖĞ¶Ï¿ªÊ¼·ÃÎÊÈ«¾Ö±äÁ¿:
	tickCounter++;
...
//·¢ÉúÊ±ÖÓÖĞ¶ÏÊ±µÄ´¦Àíº¯Êı, ¾ÍÏñÊÇpendSVÒ»Ñù,ÊÇÏµÍ³»á×Ô¶¯µ÷ÓÃµ½ÕâÀïµÄ,×¢Òâº¯ÊıÃû²»ÒªĞ´´í
...
//3.1 ¼ÇµÃÒª¶ÔÕâ¸öº¯Êı½øĞĞĞŞ¸Ä, ¾ÍÊÇÕû¸öº¯Êı¶¼ÒªÓÃÁÙ½çÇø±£»¤, ÒòÎªcurrentTaskĞèÒª±»±£»¤
void tTaskDelay(uint32_t delay)
{
	//3.1 ÉèÖÃÁÙ½çÇø,¹Ø±ÕÖĞ¶Ï
	uint32_t status = tTaskEnterCritical();
	
	currentTask->delayTicks = delay; //Ò²¾ÍÊÇÎªcurrentTaskÉèÖÃĞèÒªÑÓÊ±¶à¾Ã
	
	tTaskExitCritical(status); //×¢ÒâÔÚÕâÀï¿ªÆôÖĞ¶Ï,¶ø²»ÊÇÔÚtTaskSched()Ö®ºó,¶øÊÇÔÚËüÖ®Ç°¼ÓÕâÒ»¾ä, ÒòÎªtTaskSched()Ò²»áÉèÖÃÁÙ½çÇø
...
void task1Entry(void * param) //ÉèÖÃtask1ÈÎÎñĞèÒªÖ´ĞĞµÄº¯Êı
{
	...
	tSetSysTickPeriod(1);//ÒòÎªÖ®ºó¾Í¶¼ÔÚfor loopÀïÃæÁË,ÕâÀï¾ÍÖ»ÊÇÔËĞĞÁËÒ»´Î
	/*
	...
	*/
	...
	for(;;){
		...
		//3.1 ½øÈëÁÙ½çÇø
		uint32_t status = tTaskEnterCritical();
		//3.1 ÒÔÏÂÊÇ·ÃÎÊÈ«¾Ö±äÁ¿
		uint32_t temp = tickCounter;
		
		//ÉèÖÃÒ»¸öfor loop,Ò²¾ÍÊÇµ¢ÎóÊ±¼ä,ÈÃÖĞ¶ÏÔÚÕâ¸öÖĞ¼ä·¢Éú
		uint32_t i;
		for(i = 0; i < 0xFFFFF; i++){}
